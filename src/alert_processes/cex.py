...
class CEXAlertProcess(BaseAlertProcess):
    ...
    def get_latest_price(self, token_pair: str, retry_delay: int = 2, maximum_retries: int = 5, _try: int = 1) -> float:
        ...
        try:
            ...
            logger.error("Caught the exception during the price pulling...", exc_info=err)
    ...
    def get_pct_change(self, token_pair: str, window: str, retry_delay: int = 2, maximum_retries: int = 5,
        ...
        try:
            ...
            logger.error("Caught the exception during the pct change pulling...", exc_info=err)
            if _try == maximum_retries:
                raise ConnectionAbortedError(f'Binance request ({url}) failed after {_try} retries - Error: {err}')
            else:
                time.sleep(retry_delay)
    ...
    def tg_alert(self, post: str, channel_ids: list[str], pair: str = None) -> tuple:
        ...
        """
        post = "🔔 <b>CRYPTO LISTENER ALERT</b> 🔔\n\n" + post
        if pair:
        ...
        for g_id in channel_ids:
            for i in range(1, 3):
                try:
                    ...
                    logger.info(f"Sending the alert message for the pair {pair} in the chat [{g_id}], attempt {i}")
                    self.telegram_bot.send_message(chat_id=g_id, text=post, parse_mode="HTML",
                                                   disable_web_page_preview=True)
                    output[0].append(g_id)
                    break
                except Exception as exc:
                    logger.error("Exception during the alert message send", exc_info=exc)
