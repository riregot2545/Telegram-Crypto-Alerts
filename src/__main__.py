...
if __name__ == "__main__":
    ...
    telegramThread = TelegramCryptoBotWorker(taapiio_process)
    ...
    threading.Thread(target=CEXAlertProcess(telegram_bot=telegramThread.bot).run, daemon=True).start()
    ...
    if taapiio_process:
        ...
        threading.Thread(target=TechnicalAlertProcess(telegram_bot=telegramThread.bot).run,
                         daemon=True).start()
    ...
    while True:
        try:
            ...
            logger.info("Some error occurred, exit")